                    ...
                    <div class="formulario-busqueda">
                        ...
                        <form @submit.prevent="buscarProducto" class="mb-4">
                            <div class="search-container">
                                <div class="mb-3">
                                    <label for="num_serie" class="form-label"></label>
                                    <input v-model="num_serie" id="num_serie" class="form-control input-lg"
                        ...
                        <div v-if="busquedaRealizada && !productoEncontrado" class="alert alert-danger mt-4">
                            <p>{{ campoVacio || 'No se encontró ningún producto con ese número de serie.' }}</p>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        buscarProducto() {
            if (this.num_serie.trim() === '') {
                this.campoVacio = 'Por favor, ingrese el número de serie del producto.';
                setTimeout(() => {
                    this.campoVacio = null;
                }, 3000); 
                return;
            ...
            this.$axios.get(`http://localhost:8080/productos/traerxnumserie/${this.num_serie}`)
                .then(response => {
                    if (response.data) {
                        ...
                        this.error = null;
                        this.busquedaRealizada = true;
                        this.campoVacio = 'No se encontró ningún producto con ese número de serie.';
                        setTimeout(() => {
                            this.campoVacio = null;
                        }, 3000); 
                    }
